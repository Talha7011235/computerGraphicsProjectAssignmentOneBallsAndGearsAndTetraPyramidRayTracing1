...
{
	float determinantOfMatrix = Vector3::dot(a,b);
...
std::istream& operator>> (std::istream& in, Polygon& value)
{
	...
	// To improve hidden surface removal, include a calculation for the Square Radius that is represented by the Floating Point Private Member
	// Variable "float squareRadius" from the Header File "Polygon.h".
	...
	else
	{
		...
		for (int iIndex = 0; iIndex < value.count; ++iIndex)
		{
			...
			angles.push_back(getAngle(AC, AB));
			a = iIndex;
		}
		...
		for (int iIndex = 0; iIndex < value.count - 2; ++iIndex)
		{
			...
			angles[vertexTwo] = getAngle(vertexTwoa, vertexTwob);
			...
			angles[vertexThree] = getAngle(vertexThreea, vertexThreeb);
...
bool Polygon::Intersect(const Vector3& origin, const Vector3& direction, float& tValueIsTheDepth,float near) const
{
	...
	if (line.squareLength() - squareRadius > (near * near * 0.99f))
	{
		return false;
	}
	...
	for(int iIndex = 0; iIndex < indicies.size(); iIndex += 3)
	{
		...
		if (IntersectTriangle(indicies[iIndex], indicies[iIndex + 1], indicies[iIndex + 2], origin, direction, innerDepth))
		{
...
bool Polygon::IntersectTriangle(const int vZero, const int vOne, const int vTwo, Vector3 origin, Vector3 direction, float& tValue) const
{
